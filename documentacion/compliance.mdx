...
Dependiendo de cómo te integres, tendrás 2 endpoints para enviar esta información o para verificar el estado KYC de un usuario:
- <b>SetProfile</b>: un punto final que recibe y almacena toda la información relevante para un usuario mencionado anteriormente.
  - Este endpoint rechazará cambios en la información permanente de una persona. Es decir: si intentas actualizar el número de documento de una persona existente, la solicitud será rechazada y tendrás que contactarnos si es necesario.
  - También podrás utilizar este punto final para crear nuevos usuarios.
- <b>CheckProfile</b>: un endpoint para verificar el estado de un usuario. Para proteger la información del usuario, solo devolverá si cada uno de los atributos están correctamente registrados, en qué país puede operar el usuario y si puede operar.
