...
<Accordion title="TopUp Get">
#### Get TopUps on PaymentOrderOutputPaginated
Proporciona información sobre los topUps creados, para clientes con configuración de Settlement Asíncrono.
```graphql
query GetTopUps {
  getTopUps {
    ... on PaymentOrderOutputPaginated {
      pagination {
        totalCount
        pageSize
        pageNumber
...
```
#### Get TopUps on OrderOutput
Proporciona información sobre el topUp con el orderId ingresado, para clientes con configuración de Settlement Asíncrono.
```graphql
query GetTopUps($orderId: String) {
  getTopUps(orderId: $orderId) {
    ... on OrderOutput {
      orderId
      quoteId
      orderType
...
"variables":
{
  "orderId": "518b441f-f643-4ce7-b979-7b346b2f3ffd"
}
```
<Warning>
...
<Accordion title="TopUps Create">
#### CreateTopUps
Permite crear TopUps, requiere que se realice una cotizacion y se espera como variable el quoteId, para clientes con configuración de Settlement Asíncrono.
```graphql
mutation CreateTopUps($input: OrderInput!) {
  createTopUps(input: $input) {
    orderId
    quoteId
    symbolOut
    symbolIn
...
{
  "input": {
    "quoteId": "6696beb139b613706bbe3786"
  }
}
```
<Warning>
