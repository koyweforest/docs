...
Si updateDocumentNumber está presente, la API valida que tenga un formato válido, que no exista en otra cuenta, que no tenga orders o deals ejecutados y ejecuta una validación asíncrona en Metamap
para corroborar que los datos personales sean correctos.
 Para obtener los resultados de esa validación la Metaacount asociada a esa cuenta tiene que tener configurado un nuevo callback llamado
...
Devuelve información de 'deals' para el usuario autenticado. Recibe un filtro `status`, que acepta PAID | EXPIRED | PENDING ; un filtro de fecha desde y fecha hasta, además del limite de registros y pagina para paginado.
Este endpoint no está disponible para todos los clientes, por favor contactenos a contacto@koywe.com para más información.
