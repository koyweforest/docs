...
Updates user data. If update email is present, the API validate that is a valid email and that already do not exist another account with that email.
If update document number is present, the API validates that has a valid format, that do not exist on another account, that has no orders or deals executed and runs an async
validation on metamap for missmatching personal data. In order to get the results of that validation the Metaaccount associated to that account has to have configured a new callback called
updateDocumentNumberCallback
...
Requires: `destinationAddress`, `quoteId o symbolIn, symbolOut, amountIn, amountOut, y paymentMethodId`.

#### Off ramp

Requires: `destinationAddress`, `quoteId o symbolIn, symbolOut, amountIn, amountOut`.
...
Returns a list of bank accounts the users has associated, filtered using `countryCode` and `currencySymbol`.
...
Returns deals information for authenticated user. Receives a `status` filter, accepts PAID | EXPIRED | PENDING A date from and date to filter, as well as a record limit and page for paging.
This endpoint is not available for every client, please contact us to contacto@koywe.com for more info

```graphql
query getDeals($input: ScheduledDealsInput!) {
  getOrder(input: $input){
scheduledDealdId
    orderIds
    amount
