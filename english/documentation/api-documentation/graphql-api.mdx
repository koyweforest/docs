...
Provides balance information for 'On', 'Off', and 'TopUp' ops, for clients with Async Settlement configuration.
```graphql
...
**This method is only available for specific clients**
If you're interested in learning more about this and its use cases, please write us at: [contacto@koywe.com](mailto:contacto@koywe.com)!
  </Warning>
...
Provides information about the topUps created, for clients with Asynchronous Settlement configuration.
```graphql
query GetTopUps {
  getTopUps {
...
Provides information about the topUp with the entered orderId, for clients with Asynchronous Settlement configuration.
```graphql
...
#### CreateTopUps
It allows you to create TopUps, requires that a quote be made and the quoteId is expected as a variable, for clients with Asynchronous Settlement configuration.
```graphql
