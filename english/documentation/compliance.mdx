...
- SetProfile: an endpoint that receives and stores all the relevant information for a user listed above.
...
- CheckProfile: an endpoint to check the status of a user. To protect the user's information, it will only return if each of the attributes are correctly registered, which country the user can operate, and if it can operate.
